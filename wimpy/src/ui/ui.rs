...
pub struct UINodeState<TLayout> {
    layout: TLayout,
    uv: Area,
    rotation: f32,
    color: Color,
...
impl<TLayout> Default for UINodeState<TLayout> where TLayout:Default {
    fn default() -> Self {
        Self {
            layout: TLayout::default(),
            uv: Area::ONE,
            rotation: 0.0,
            color: Color::WHITE,
            texture: None,
...
pub struct Page<TTexture> {
    node_container: NodeContainer<UINodeState<Layout>,UINodeState<Area>>,
...
impl<TTexture> Page<TTexture> {
    ...
    pub fn create_node(&mut self,node: UINodeState<Layout>) -> Node {
    ...
    // TODO: Interface the renderer here
}
