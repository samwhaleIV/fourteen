    ...
    IO: WimpyIO
{
    ...
    fn update(&mut self,context: &mut WimpyContext) {
        ...
        let mut output = match context.graphics.create_output_builder(WimpyColor::MAGENTA) {
            Ok(value) => value,
            Err(surface_error) => {
        ...
        'first_pass: {
            ...
            let layout = WimpyLayout {
                x: LayoutDimension {
                    ...
                    size: Size::of_parent_height(0.10),
                    size_offset: 0.into(),
                },
                y: LayoutDimension {
                    position: Position::center_of_parent(),
            ...
            render_pass.set_sampler_mode(SamplerMode::LinearClamp);
...
impl GraphicsContextConfig for PlaceholderConfig {
    ...
    const UNIFORM_BUFFER_SIZE: usize = mb_to_b(5);
